...
function addRow(table) {
    ...
    removeCell.innerHTML = '<button type="button">حذف</button>';
...
document.getElementById('testForm').addEventListener('submit', function (event) {
    ...
    if (isNaN(totalStudents) || totalStudents <= 0) {
        alert('الرجاء إدخال عدد صحيح إيجابي لإجمالي الطلاب');
    ...
    for (var i = 1; i < rows.length; i++) {
        ...
        if (isNaN(studentValue) || studentValue <= 0 || studentValue>totalStudents) {
            alert('الرجاء إدخال عدد صحيح إيجابي للطلاب الذين حققوا الهدف في الصف'+" "+i);
